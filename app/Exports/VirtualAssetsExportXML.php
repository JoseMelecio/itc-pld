...
class VirtualAssetsExportXML
{
    ...
    public function makeXML()
    {
        ...
        $xmlObject->startElement('archivo');
            $xmlObject->writeAttribute('xsi:schemaLocation', 'http://www.uif.shcp.gob.mx/recepcion/avi avi.xsd');
            $xmlObject->writeAttribute('xmlns', 'http://www.uif.shcp.gob.mx/recepcion/avi');
        ...
        $xmlObject->startElement('dominio_plataforma');
            $xmlObject->text(Str::upper($this->headers['platform_domain']));
        ...
        foreach ($this->data['items'] as $key => $notice) {
            ...
            $xmlObject->startElement('operaciones_persona');
            $xmlObject->startElement('persona_aviso');

            $xmlObject->startElement('datos_cuenta_plataforma');
                $xmlObject->startElement('id_usuario');
                    $xmlObject->text($notice['identificationDataPersonSubjectNotice']['platformData']['id_user']);
                $xmlObject->endElement(); // id_usuario

                $xmlObject->startElement('cuenta_relacionada');
                    $xmlObject->text($notice['identificationDataPersonSubjectNotice']['platformData']['account_related']);
                $xmlObject->endElement(); // cuenta_relacionada

                $xmlObject->startElement('clabe_interbancaria');
                    $xmlObject->text($notice['identificationDataPersonSubjectNotice']['platformData']['clabe']);
                $xmlObject->endElement(); // clabe_interbancaria

                $xmlObject->startElement('moneda_cuenta');
                    $xmlObject->text($notice['identificationDataPersonSubjectNotice']['platformData']['currency']);
                $xmlObject->endElement(); // moneda_cuenta

            $xmlObject->endElement(); // datos_cuenta_plataforma
            ...
            if (strlen($notice['identificationDataPersonSubjectNotice']['physicalPerson']['name']) > 0) {
                ...
                $xmlObject->startElement('documento_identificacion');

                    $xmlObject->startElement('tipo_identificacion');
                        $xmlObject->text($notice['identificationDataPersonSubjectNotice']['physicalPerson']['identification_type']);
                    $xmlObject->endElement(); // tipo_identificacion

                    $xmlObject->startElement('numero_identificacion');
                        $xmlObject->text($notice['identificationDataPersonSubjectNotice']['physicalPerson']['identification_id']);
                    $xmlObject->endElement(); // numero_identificacion

                $xmlObject->endElement(); // documento_identificacion
            ...
            elseif (strlen($notice['identificationDataPersonSubjectNotice']['legalPerson']['company_name']) > 0) {
                ...
                $xmlObject->startElement('representante_apoderado');
                    ...
                    $xmlObject->startElement('documento_identificacion');
                        ...
                        $xmlObject->startElement('tipo_identificacion');
                            $xmlObject->text($notice['identificationDataPersonSubjectNotice']['representativeData']['identification_type']);
                        $xmlObject->endElement(); // tipo_identificacion

                        $xmlObject->startElement('numero_identificacion');
                            $xmlObject->text($notice['identificationDataPersonSubjectNotice']['representativeData']['identification_id']);
                        $xmlObject->endElement(); // numero_identificacion
            ...
            $xmlObject->startElement('detalle_operaciones');
                ...
                foreach ($notice['operationDetails'] as $clientOperations)
                {
                    //BUY
                    if (count($clientOperations['buysell']['COMPRA']) > 0) {
                        $xmlObject->startElement('operaciones_compra');

                        foreach ($clientOperations['buysell']['COMPRA'] as $operationBuy) {
                            $xmlObject->startElement('compra');

                                $xmlObject->startElement('fecha_hora_operacion');
                                    $xmlObject->text($operationBuy['date_time_operation']);
                                $xmlObject->endElement(); // fecha_hora_operacion

                                $xmlObject->startElement('moneda_operacion');
                                    $xmlObject->text($operationBuy['operation_currency']);
                                $xmlObject->endElement(); // moneda_operacion

                                $xmlObject->startElement('monto_operacion');
                                    $xmlObject->text(number_format($operationBuy['operation_amount'], 2, '.', ''));;
                                $xmlObject->endElement(); // monto_operacion

                                $xmlObject->startElement('activo_virtual');

                                    $xmlObject->startElement('activo_virtual_operado');
                                        $xmlObject->text($operationBuy['virtual_asset']);
                                    $xmlObject->endElement(); // activo_virtual_operado

                                    if ($operationBuy['virtual_asset'] == '999999') {
                                        $xmlObject->startElement('descripcion_activo_virtual');
                                            $xmlObject->text($operationBuy['virtual_asset_description']);
                                        $xmlObject->endElement(); // descripcion_activo_virtual
                                    }

                                    $xmlObject->startElement('tipo_cambio_mn');
                                        $xmlObject->text(number_format($operationBuy['exchange'], 2, '.', ''));
                                    $xmlObject->endElement(); // tipo_cambio_mn

                                    $xmlObject->startElement('cantidad_activo_virtual');
                                        $xmlObject->text(number_format($operationBuy['quantity_virtual_asset'], 2, '.', ''));
                        ...
                        $xmlObject->endElement(); // operaciones_compra
                    }

                    //SELL
                    if (count($clientOperations['buysell']['VENTA']) > 0) {
                        $xmlObject->startElement('operaciones_venta');

                        foreach ($clientOperations['buysell']['COMPRA'] as $operationSell) {
                            $xmlObject->startElement('venta');

                            $xmlObject->startElement('fecha_hora_operacion');
                                $xmlObject->text($operationSell['date_time_operation']);
                            $xmlObject->endElement(); // fecha_hora_operacion

                            $xmlObject->startElement('moneda_operacion');
                                $xmlObject->text($operationSell['operation_currency']);
                            $xmlObject->endElement(); // moneda_operacion

                            $xmlObject->startElement('monto_operacion');
                                $xmlObject->text(number_format($operationSell['operation_amount'], 2, '.', ''));
                            $xmlObject->endElement(); // monto_operacion

                            $xmlObject->startElement('activo_virtual');

                                $xmlObject->startElement('activo_virtual_operado');
                                    $xmlObject->text($operationSell['virtual_asset']);
                                $xmlObject->endElement(); // activo_virtual_operado

                                if ($operationSell['virtual_asset'] == '999999') {
                                    $xmlObject->startElement('descripcion_activo_virtual');
                                        $xmlObject->text($operationSell['virtual_asset_description']);
                                    $xmlObject->endElement(); // descripcion_activo_virtual
                                }

                                $xmlObject->startElement('tipo_cambio_mn');
                                    $xmlObject->text(number_format($operationSell['exchange'], 2, '.', ''));
                                $xmlObject->endElement(); // tipo_cambio_mn

                                $xmlObject->startElement('cantidad_activo_virtual');
                                    $xmlObject->text(number_format($operationSell['quantity_virtual_asset'], 2, '.', ''));
                                $xmlObject->endElement(); // cantidad_activo_virtual
            ...
            $xmlObject->endElement(); // operaciones_persona
        }
