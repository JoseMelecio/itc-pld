...
class CreditAndServiceCardExportXML
{
    private array $data;
    ...
    public function makeXML()
    {
        ...
        $xmlObject->startElement('archivo');
            $xmlObject->writeAttribute('xsi:schemaLocation', 'http://www.uif.shcp.gob.mx/recepcion/tsc tsc.xsd');
            $xmlObject->writeAttribute('xmlns', 'http://www.uif.shcp.gob.mx/recepcion/tsc');
        ...
        foreach ($this->data['items'] as $key => $notice) {
            ...
            $xmlObject->startElement('detalle_operaciones');
                ...
                foreach ($notice['operationDetails'] as $operations)
                {
                    foreach ($operations as $operation)
                    {
                        $xmlObject->startElement('datos_operacion');

                        $xmlObject->startElement('fecha_periodo');
                        $xmlObject->text($operation['period_report']);
                        $xmlObject->endElement(); // fecha_periodo

                        $xmlObject->startElement('tipo_operacion');
                        $xmlObject->text($operation['operation_type']);
                        $xmlObject->endElement(); // tipo_operacion

                        $xmlObject->startElement('tipo_tarjeta');
                        $xmlObject->text($operation['card_type']);
                        $xmlObject->endElement(); // tipo_tarjeta

                        $xmlObject->startElement('numero_identificador');
                        $xmlObject->text($operation['account']);
                        $xmlObject->endElement(); // numero_identificador

                        $xmlObject->startElement('monto_gasto');
                        $xmlObject->text(number_format($operation['amount'], 2, '.', ''));
                        $xmlObject->endElement(); // monto_gasto
